### CREATING A CUSTOM MODEL RUNNER
...
bento_model = bentoml.sklearn.get("my_beautiful_model:latest")

def Extract(lst):
    # return [item[-1] for item in lst][0]
    return [item[-1] for item in lst][0]*100
...
class Heartpredict(bentoml.Runnable):
    ...
    def __init__(self):
        ...
        self.model = bentoml.sklearn.load_model("my_beautiful_model:latest")

    @bentoml.Runnable.method(batchable=False)
    def predict_proba(self, input_data):

        encoder = pickle.load(open("src/processors/encoder.pkl", "rb"))
